...
void test(int philosopherId)
{
	if (state[philosopherId] == HUNGRY && state[LEFT] != EATING ) 
	{
	   ...
	   if(philosopherId != 0)
	   {
	   printf("Philosopher %d picks fork %d and bowl %d\n",philosopherId + 1, LEFT + 1,philosophers[philosopherId]);
...
void put_down_fork(int philosopherId)
{
	...
	if(philosopherId != 0)
	{
	printf("Philosopher %d putting fork %d and bowl %d down on the table.\n",philosopherId + 1, LEFT + 1,philosophers[philosopherId]);
	}
